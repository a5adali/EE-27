...
def remove_supply_link(graph, warehouse_o, warehouse_i):
    ...
    #find and kill the connection
     for i in graph:
        if i[0] == warehouse_o:  #found source warehouse
            #look through its connections
            for j in graph[i]:
                if j[0] == warehouse_i:  #found target
