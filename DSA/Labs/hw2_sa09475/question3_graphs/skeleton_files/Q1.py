...
def list_of_vertices(filename):
    ...
    with open(filename, mode = 'r') as file:
        ...
        for line in file.readlines():
            if (line.strip().split(',')[0],float(line.strip().split(',')[-1])) not in vertices:
                vertices.append((line.strip().split(',')[0],float(line.strip().split(',')[-1])))
    return vertices
...
def list_of_edges(filename):
    ...
    with open(filename, mode = 'r') as file:
        ...
        for line in file.readlines():
            ...
            line[:len(line)-1][2],line[:len(line)-1][3] = int(line[:len(line)-1][2]),float(line[:len(line)-1][3])
            edges.append(tuple(line[:len(line)-1]))
    return edges
...
def create_supply_chain(filename):
    ...
    # provide implementation here
    G = {}
    addVertices(G,list_of_vertices(filename))
    addEdges(G,list_of_edges(filename))
    return G
