...
def add_warehouse(graph, warehouse_o, tr_trust, warehouse_i, weight):
   ...
   #check if new warehouse already exists
   if search_warehouse(graph, warehouse_o):
   ...
   if not search_warehouse(graph, warehouse_i):
        ...
        return  
    #add the warehouse to the graph with its trust value
   addVertices(graph, [(warehouse_o, tr_trust)])
    #connect the new warehouse to the existing one
